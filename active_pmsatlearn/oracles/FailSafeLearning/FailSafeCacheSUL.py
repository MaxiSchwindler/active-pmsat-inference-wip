...
from .Errors import NonDeterministicError, RepeatedNonDeterministicError, TableError
...
# -- ble-learning/FailSafeLearning/FailSafeCacheSUL.py
#
# Adaptions to the existing code have been made:
# -- unify with FailSafeSUL (add 'performed_steps_in_query' attr to sul directly in this file)
# -- add getattr() to access attrs of underlying sul
# -- added colorama_init(autoreset=True) to avoid coloring _all_ output
###
...
class FailSafeCacheSUL(SUL):
    ...
    def __init__(self, sul):
        super().__init__()
        self.sul = sul
        self.cache = CacheTree(constants.NON_DET_CACHE_SIZE)
        self.non_det_query_counter = 0
        self.non_det_step_counter = 0
        self.reset_time = 0
        self.physical_reset = 0
        self.sul.performed_steps_in_query = 0
    ...
    def query(self, word):
        ...
        attempts = 0
        while attempts < constants.NON_DET_ERROR_ATTEMPTS:
            try:
                non_det_update = 0
                while non_det_update < constants.NON_DET_CACHE_SIZE:
                    try:
                        ...
                        self.cache.reset()
                        for i, o in zip(word, out):  # TODO doesnt this anyways happen in step()??
                            self.cache.step_in_cache(i, o)
                        non_det_update = constants.NON_DET_CACHE_SIZE
                        return out
                ...
                if attempts == constants.NON_DET_ERROR_ATTEMPTS:
