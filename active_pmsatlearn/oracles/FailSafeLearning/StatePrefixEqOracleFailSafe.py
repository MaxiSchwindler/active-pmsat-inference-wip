import random
import active_pmsatlearn.oracles.FailSafeLearning.constants as constants

from .Errors import ConnectionError, NonDeterministicError, RepeatedNonDeterministicError
...
# -- ble-learning/FailSafeLearning/StatePrefixEqOracleFailSafe.py
#
###
...
class StatePrefixOracleFailSafe(StatePrefixEqOracle):
    ...
    def repeat_query(self, hypothesis, input_sequence):
        ...
        non_det_attempts = 0
        while non_det_attempts < constants.NON_DET_ERROR_ATTEMPTS:
    ...
    def find_cex(self, hypothesis):
        hypothesis.compute_prefixes()  # ADDED: ensure prefixes are computed
        ...
        for state in states_to_cover:
            ...
            out_sul = constants.ERROR
            non_det_attempts = 0
            error_counter = 0
            while out_sul == constants.ERROR and error_counter < constants.CONNECTION_ERROR_ATTEMPTS and non_det_attempts < constants.NON_DET_ERROR_ATTEMPTS:
                try:
                    self.reset_hyp_and_sul(hypothesis)
                    out_sul = ''
                    prefix = state.prefix
                    ...
                    if error_counter >= constants.CONNECTION_ERROR_ATTEMPTS and out_sul == constants.ERROR:
                        start_time = time.time()
                        inp = input(
                            "Repeated connection error! Cancel execution with 'c'. Otherwise, reset the device and press any other key.")
                        if inp == 'c' or inp == 'C':
                            raise
                        else:
                    ...
                    if non_det_attempts == constants.NON_DET_ERROR_ATTEMPTS:
                        start_time = time.time()
                        inp = input(
                            "Repeated non-deterministic error! Cancel execution with 'c'. Otherwise, reset the device and press any other key.")
                        if inp == 'c' or inp == 'C':
                            raise
                        else:
                            self.physical_reset += 1
